...
void AMyProjectBall::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindTouch(IE_Pressed, this, &AMyProjectBall::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &AMyProjectBall::TouchStopped);
}
...
void AMyProjectBall::Jump()
{
	if(bCanJump)
	{
		...
		Ball->BodyInstance.MassScale -= .1f;
...
void AMyProjectBall::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
	if (bCanJump)
	{
		const FVector Impulse = FVector(0.f, 0.f, JumpImpulse);
...
void AMyProjectBall::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
	if (bCanJump)
	{
		const FVector Impulse = FVector(0.f, 0.f, JumpImpulse);
